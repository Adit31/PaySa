...
int checkbalance(struct PaySa* x)
{
if(x->balance>x->bank.bal||x->balance<50)
{
...
{
cscreen();
if((x->zr==9)||(x->zf==x->zr+1))
{
printf("Cannot make request, receiver's in box is currently full\n");
getch();
return 0;
}
else if(x->zf==-1)
